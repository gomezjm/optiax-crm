...
| R3 | **built + verified live** (332 unit, db:test green incl. gate; live probes n=5) — pending Juan's review + merge | `feat/ws-r3-evals` | 5 answers ratified → R3 spec §6. R2 Q-C/Q-D **closed with data** (both dropped). §0 handoff-ceiling defect fixed. `evaluateDraft` gate ready for D3. |
| D3 | spec + brief ready; starts after R3 merges | — | **Runtime + dashboard** (owns the one true FE↔BE call: playground draft-mode). Configurator wizard + playground + publish flow (wires `evaluateDraft`). **Carry-overs**: `/inicio`→`/home` (A); relabel "Total gastado"→"Total en pedidos" (F); composer `order_items.sort_order` (R2 Q-A); recompile-published-prompts script. |
| D4 | queued after D3 | — | Home KPIs + Settings masters. **`orders.verified_by`** (B); revisit `agent_turns.prompt_version_id` nullability. **Phase 4**: thread `tenants.timezone` through dashboard date helpers (D). **Backlog**: editable order items (C). |
