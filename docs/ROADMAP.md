...
| D2 | **merged** into `main` (`c9c26da`) — unblocked R2 | `feat/ws-d2-orders-products` | 31 assumptions + 7 answers ratified → D2 spec §7. One migration (`total_spent` trigger); seed reconciled. |
| R2 | **built + verified live** (319 unit, 273 db tests ✓) — pending Juan's review + merge | `feat/ws-r2-agent-tools` | 26 assumptions + 5 questions → SESSION_NOTES. Carry-overs 0.1–0.4 landed separately. One migration (`order_items.sort_order`). **`COMPILER_VERSION` 1.0.0 → 1.1.0** (tool instructions + `capture_lead`→`capture_customer`) — prompts must be recompiled on deploy. |
| R3, D3, D4 | queued — sequential: R3 → D3 → D4 | — | **D3 carry-overs**: rename `/inicio`→`/home` (A); relabel "Total gastado"→"Total en pedidos" (F); dashboard composer writes `order_items.sort_order` (R2 Q-A). **D4**: `orders.verified_by` (B); revisit `agent_turns.prompt_version_id` nullability. **Phase 4**: thread `tenants.timezone` through dashboard date helpers (D). **Backlog**: editable order items (C). **R2 open**: tool-result persistence across messages (Q-C). |
