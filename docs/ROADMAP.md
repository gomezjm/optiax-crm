...
| D2 | **built + verified live** (242 unit, 270 db tests, prod build ✓) — staged, pending Juan's go-ahead to commit + review | `feat/ws-d2-orders-products` | 31 assumptions + 7 answers ratified → D2 spec §7. One migration (`total_spent` trigger); seed reconciled. |
| R2 | spec + brief ready; starts after D2 merges | — | **Carry-overs**: `no_published_config` skip reason; `schedule` required for `outside_hours`; `packages/shared` subpath-export split; `order_items.sort_order` migration (E). Reuses D2's `OrderCreateSchema`/`ProductSchema`. |
| R3, D3, D4 | queued — sequential: R2 → R3 → D3 → D4 | — | **D3 carry-overs**: rename `/inicio`→`/home` (A); relabel "Total gastado"→"Total en pedidos" (F). **D4**: `orders.verified_by` (B); revisit `agent_turns.prompt_version_id` nullability. **Phase 4**: thread `tenants.timezone` through dashboard date helpers (D). **Backlog**: editable order items (C). |
