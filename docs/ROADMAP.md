...
| D4 | spec + brief ready; **last Phase 2 session**; starts after D3 merges | — | Home KPIs (Screen 0) + Settings masters (Screen 7). **Carry-ins**: `orders.verified_by` (D2-B); in-app nav guard for `/agent` (D3-D); optional `agent_turns.prompt_version_id` nullability for tenant-health. **Phase 4**: `tenants.timezone` through dashboard date helpers (D2-D). **Backlog**: editable order items (C); partial-draft persistence (D3-B). |
